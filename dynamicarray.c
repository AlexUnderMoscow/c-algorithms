...
*/
void initDynArr(DynamicArray *v, int capacity)
{
	assert(capacity > 0);
	assert(v!= 0);
...
{
	DynamicArray *r = (DynamicArray *)malloc(sizeof( DynamicArray));      //Variable decalaration must occur before asserting
	assert(cap > 0);
	assert(r != 0);
	initDynArr(r,cap);
	r->arraySize = sizeDynArr;
	r->add = addDynArr;
	r->get =  getDynArr;
	r->put = putDynArr;
	r->swap = swapDynArr;
	r->remove = removeAtDynArr;
	r->isEmpty = isEmptyDynArr;
	r->push = pushDynArr;
	r->top = topDynArr;
	r->pop = popDynArr;
	r->contains = containsDynArr;
	r->removeAt = removeDynArr;
	return r;
}
...
*/
void freeDynArr(DynamicArray *v)
{
	if(v->data != 0)
	{
...
*/
void _dynArrSetCapacity(DynamicArray *v, int newCap)
{
        int i = 0;
...
void removeDynArr(DynamicArray *v, TYPE val)
{
   ...
   for (i = 0; i < v->size; i++) {
      if (EQ(val, v->data[i])) { /* found it */
         //Overwrite each element with value from successive element starting at index
         for(j = i; j < v->size - 1; j++)     //Size - 1 is the index for the last data in array
            v->data[j] = v->data[j + 1];
         //Set last element data to NULL
         v->data[v->size - 1] = 0;    //Size - 1 is the index for the last data in array
